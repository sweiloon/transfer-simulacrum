...
const TransferLoading = () => {
  ...
  useEffect(() => {
    ...
    // Start progress increment every 30 seconds
      ...
      <div className="max-w-6xl mx-auto pt-8">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className={`font-semibold ${style.text}`}>
                      {transferData.date.toLocaleDateString()}
                    </p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-600">Time</label>
                    <p className={`font-semibold ${style.text}`}>{transferData.time}</p>
                  </div>
                </div>
        ...
        <div className="mt-6 p-4 bg-white/60 backdrop-blur-sm rounded-xl border border-gray-200">
          <p className="text-sm text-gray-600 text-center">
            🔒 This transaction is secured with bank-grade encryption. Please do not refresh or close this page until completion.
          </p>
        </div>
      </div>
    </div>
